                    ...
                    server_rec *s)
{
    ...
    /* Add version string to Apache headers checking PRIMARY server config (global) */
    if (pConfig->bAnnounce) {
        ap_add_version_component(p, PACKAGE_NAME"/"PACKAGE_VERSION);
    }

    /* Check PASV Address Override settings */
    for (ls = s; ls; ls = ls->next) {
        pConfig = ap_get_module_config(ls->module_config, &ftpd_module);
        if (pConfig->sPasvAddr && pConfig->sPasvAddrExclusion) {
            char *tmp, *tmp2;
            tmp2 = ap_strchr(pConfig->sPasvAddrExclusion,'/');
            tmp = apr_pstrndup(p,pConfig->sPasvAddrExclusion,tmp2 - pConfig->sPasvAddrExclusion);
            tmp2++;
            apr_ipsubnet_create(&(pConfig->pPasvAddrExclusions),tmp,tmp2,p);
        }
    }
...
static const char *ftpd_set_enabled(cmd_parms *cmd, void *dummy, int arg)
{
...
static const command_rec ftpd_cmds[] = {
    AP_INIT_FLAG("FtpProtocol", ftpd_set_enabled,
                NULL, RSRC_CONF,
    ...
    AP_INIT_TAKE1("FtpPasvOverrideAddr", ap_set_server_string_slot,
                (void *)APR_OFFSETOF(ftpd_svr_config_rec, sPasvAddr), RSRC_CONF,
                "Override the Passive source port (the ftp server) with this IP. Default: none, auto detect based on incoming connection"),

    AP_INIT_TAKE1("FtpPasvOverrideAddrExclusion", ap_set_server_string_slot,
                (void *)APR_OFFSETOF(ftpd_svr_config_rec, sPasvAddrExclusion), RSRC_CONF,
                "CIDR formatted (ip/{mask or # of bits) netblock that will NOT be overriden when FtpPasvOverrideAddr is set."),
